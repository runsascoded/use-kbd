 ...
 */
export interface DynamicHotkeysConfig {
...
/**
 * Context value for dynamic hotkeys.
 */
export interface DynamicHotkeysContextValue {
...
const DynamicHotkeysContext = createContext<DynamicHotkeysContextValue | null>(null)

const DEFAULT_CONFIG: Required<DynamicHotkeysConfig> = {
  storageKey: 'hotkeys',
...
/**
 * Provider for dynamic hotkey registration via useAction.
 *
 * Unlike HotkeysProvider, this doesn't require upfront action definitions.
 ...
 * function App() {
 *   return (
 *     <DynamicHotkeysProvider config={{ storageKey: 'my-app' }}>
 *       <Dashboard />
 *       <ShortcutsModal />
 *       <Omnibar />
 *     </DynamicHotkeysProvider>
 *   )
 * }
 *
 * function Dashboard() {
 ...
 * ```
 */
export function DynamicHotkeysProvider({
  config: configProp = {},
  children,
}: DynamicHotkeysProviderProps) {
  const config = useMemo<Required<DynamicHotkeysConfig>>(() => ({
    ...DEFAULT_CONFIG,
    ...configProp,
  }), [configProp])
  ...
  const value = useMemo<DynamicHotkeysContextValue>(() => ({
    registry,
    isEnabled,
    isModalOpen,
    openModal,
  ...
  return (
    <ActionsRegistryContext.Provider value={registry}>
      <DynamicHotkeysContext.Provider value={value}>
        {children}
      </DynamicHotkeysContext.Provider>
    </ActionsRegistryContext.Provider>
  )
}

/**
 * Hook to access the dynamic hotkeys context.
 * Must be used within a DynamicHotkeysProvider.
 */
export function useDynamicHotkeysContext(): DynamicHotkeysContextValue {
  const context = useContext(DynamicHotkeysContext)
  if (!context) {
    throw new Error('useDynamicHotkeysContext must be used within a DynamicHotkeysProvider')
  }
  return context
}

/**
 * Hook to optionally access dynamic hotkeys context.
 */
export function useMaybeDynamicHotkeysContext(): DynamicHotkeysContextValue | null {
  return useContext(DynamicHotkeysContext)
}
