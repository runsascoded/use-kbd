...
export const MobileSearchButton = () => {
  ...
  return (
    <button className="mobile-search-btn" onClick={() => openOmnibar()} aria-label="Open search">
      <FaSearch />
    </button>
  )
}

<span className="try-intro desktop-only">Try them on this page:</span>
<span className="try-intro mobile-only">Try them on this page, or tap <MobileSearchButton />:</span>
...
<div className="components-table-wrapper">
  <table className="components-table">
    <thead>
      <tr>
        <th>Keys</th>
        <th><Tooltip title="View source on GitHub"><A href="https://github.com/runsascoded/use-kbd/tree/main/site/#readme" style={{ color: 'inherit' }}><FaGithub size={20} /></A></Tooltip></th>
        <th>Description</th>
      </tr>
    </thead>
    <tbody>
...
3. **Easy theming** with CSS variables (e.g. <A href="https://github.com/runsascoded/use-kbd/blob/main/site/src/styles/_variables.scss">`_variables.scss`</A>, <A href="https://github.com/runsascoded/use-kbd/blob/main/site/src/styles/_calendar.scss">`_calendar.scss`</A>)
